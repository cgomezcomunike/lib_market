...
if(1){
	...
	$usuario_sync = MEGA_mySQL_R("SELECT * FROM `suscripcion_post` WHERE `campo_1` = '".$usuario."' AND (`estado` = '2' OR `estado` = '1' ) AND campo_2 != '' AND perfil != '' ORDER BY `id` DESC LIMIT 1; ","id",$modular_dir."/config/".$modular_ap_name_config.".php");


	$consulta = mysql_query("SELECT * FROM `requerimientos` WHERE `Estado` != 'Finalizado' AND `Estado` != 'Abortado' AND `id` != '0' AND `papelera` = '0' AND `temp` = '0' AND `personal` LIKE '%-".$usuario_sync."-%' AND `fecha_presupuesto` > '2021-12-31'  ORDER BY `fecha_presupuesto_timeStamp` DESC",$db);
