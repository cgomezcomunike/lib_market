	...
	const socket = new CMNKSocket({
		...
		onGet: (ev) => {
			...
			try {
				...
				if (typeof data.reaction !== "object") return;
				for (const k in data.reaction) {
					const split = k.split("/");
					const route = split[0];
					const action = split[1];
					const v = data.reaction[k];
					if (typeof reactions[route] === "undefined") return;
					if (typeof reactions[route][action] === "function") {
						reactions[route][action](v);
					}
				}
			} catch (err) {
...
const actions = {
	...
	get_group_users: (props) => {
		if (typeof props.group === "undefined") return;
		_root_data.socket.send(JSON.stringify({
			accion: "chat/Get_users_group",
			user: _root_data.user,
			cv: _root_data.cv,
			firma: _root_data.firma,
			token: _root_data.token,
...
const reactions = {
	to_me: {
		...
		bind_users_group: (data) => {
			const {users, group} = data;
			render.users_groups({users, group});
			events.users_group({group});
		}
	}
}
...
const events = {
	...
	groups: () => {
		const groups_btn = document.querySelectorAll(".btn_groups");
		for (const btn of groups_btn) {
			const key = btn.getAttribute("key");
			btn.onclick = () => {
				actions.get_group_users({group: key});
			}
		}
	},
	users_group: (props) => {
		const {group} = props;
		const btn_group_users = document.querySelectorAll(".btn_group_users");
		for (const btn of btn_group_users) {
			btn.onclick = () => {
				const key = btn.getAttribute("key");
				render.messages({to: key, group});
			}
		}
	}
}
...
const render = {
	...
	groups: (data) => {
		...
		groups.innerHTML = `<div>${Object.keys(data).map((item) => {
			return `<div>
				<button key="${item}" class="btn_groups">${item} (${data[item].length})</button>
			</div>`;
		}).join("")
			}
		</div>`;
	...
	users_groups: (data) => {
		...
		const group_users = document.querySelector("#group_users");
		group_users.innerHTML = `<div>
			<hr/>
			<h3>Usuarios - ${group}</h3>
		${users.map(item => {
			return `<div>
				<button class="btn_group_users" key="${item}">${item}</button>
			</div>`;
		}).join("")}
		</div>`;
	},
	messages: (data) => {
		...
		events.send_message_form({to: data.to, group: data.group || "alive"});
	},
	input_messages: (data) => {
